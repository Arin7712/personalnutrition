...
const Faq3 = ({ items = faqItems }: Faq3Props) => {
  ...
  return (
    <section ref={ref} className="pt-[4rem] md:px-0">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          >
            {leftItems.map((item, index) => (
              <motion.div key={item.id} {...fadeIn(isInView, index * 0.15)}>
                <AccordionItem value={item.id} className="md:w-lg pb-6">
          ...
          >
            {rightItems.map((item, index) => (
              <motion.div
                key={item.id}
                {...fadeIn(isInView, (index + leftItems.length) * 0.15)}
              >
                <AccordionItem value={item.id} className="md:w-lg pb-6">
                  <AccordionTrigger className="hover:no-underline hover:opacity-60">
                    <div className="font-medium lg:text-md">{item.question}</div>
